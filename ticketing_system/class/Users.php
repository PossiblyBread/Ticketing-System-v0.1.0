...
class Users extends Database { 
	...
	// added a new get function for getting user's name
	// public function getUserName() {
	// 	if(!empty($_SESSION["user_name"])) {
	// 		$sqlQuery = "SELECT * FROM ".$this->userTable." 
	// 			WHERE id ='".$_SESSION["user_name"]."'";
	// 		$result = mysqli_query($this->dbConnect, $sqlQuery);		
	// 		$userDetails = mysqli_fetch_assoc($result);
	// 		return $userDetails;
	// 	}		
	...
	public function listUser(){
		...
		while( $user = mysqli_fetch_assoc($result) ) {		
			...
			if($user['user_type'] == 'L-1') {
				$userRole = '<span class="label label-warning">L-1</span>';
					//$userRole = '<span class="label label-warning">Member</span>';
			}
			if($user['user_type'] == 'L-2') {
				$userRole = '<span class="label label-warning">L-2</span>';
			}
			if($user['user_type'] == 'L-3') {
				$userRole = '<span class="label label-warning">L-3</span>';
			}	
